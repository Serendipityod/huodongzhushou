...
import { 
  LayoutDashboard, Clock, ChevronDown, ChevronUp, Download, 
  Trash2, MapPin, Plus, PlayCircle, CheckCircle, XCircle, 
  Upload, FileSpreadsheet, Info, AlertTriangle, 
  ListOrdered, Check, AlertCircle, Wand2, EyeOff, 
  RotateCcw, Lightbulb, FilePlus 
} from 'lucide-react';
...
// --- CONSTANTS ---
const INITIAL_TIME_FORMATS: TimeFormatItem[] = [
...
const validateTimeFormat = (timeStr: string, formats: TimeFormatItem[]): { isValid: boolean, message?: string } => {
...
const getRecommendedTime = (input: string): string | null => {
...
const getRecommendedLocation = (input: string, library: AddressLibraryItem[]): string | null => {
  if (!input) return null;
...
const generateRegexFromTime = (input: string): string => {
...
const EventList: React.FC<EventListProps> = ({ 
  events, onDelete, onClearAll, onUpdateEvent, onSelectRow, selection
}) => {
  useEffect(() => {
              ...
              return (
                <tr 
                  ...
                  className={`transition-colors cursor-pointer ${
                    ...
                    hasAnyError ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-gray-50'
                  }`}
                >
...
const AddressLibrary: React.FC<AddressLibraryProps> = ({ locations, onAddLocation, onRemoveLocation }) => {
...
const TimeFormatLibrary: React.FC<TimeFormatLibraryProps> = ({ formats, onAddFormat, onRemoveFormat }) => {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div key={fmt.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 group">
                <div className="flex-1 min-w-0 mr-4">
                  <div className="flex items-center">
                    ...
                    {fmt.isSystem && <span className="ml-2 px-2 py-0.5 rounded text-xs font-medium bg-gray-200 text-gray-600">系统预设</span>}
                  </div>
                  <p className="text-xs text-gray-500 font-mono mt-1 truncate" title={fmt.pattern}>{fmt.pattern}</p>
                </div>
                {!fmt.isSystem && (
                  <button onClick={() => onRemoveFormat(fmt.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors">
                    <Trash2 className="w-5 h-5" />
                  </button>
                )}
              </div>
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" required className="w-full rounded-md border-gray-300 shadow-sm border px-3 py-2 focus:ring-orange-500 bg-white"
                value={newFormat.name} onChange={(e) => setNewFormat({ ...newFormat, name: e.target.value })} placeholder="例如：全数字日期" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">正则表达式 (Regex)</label>
              <input type="text" required className="w-full rounded-md border-gray-300 shadow-sm border px-3 py-2 focus:ring-orange-500 bg-white font-mono text-sm"
                value={newFormat.pattern} onChange={(e) => setNewFormat({ ...newFormat, pattern: e.target.value })} placeholder="例如：^\d{4}年$" />
            </div>
            <button type="submit" className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 transition-colors">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-4">
            <input type="text" className="w-full rounded-md border-gray-300 shadow-sm border px-3 py-2 focus:ring-indigo-500 bg-white"
              value={testValue} onChange={(e) => setTestValue(e.target.value)} placeholder="输入时间文本测试..." />
            {testValue && (
              <div className={`flex flex-col p-3 rounded-md ${isTestValid ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                <div className="flex items-center">
                  {isTestValid ? <><CheckCircle className="w-5 h-5 mr-2" /><span className="text-sm font-medium">格式匹配成功</span></> : <><XCircle className="w-5 h-5 mr-2" /><span className="text-sm font-medium">格式不匹配</span></>}
                </div>
...
const ImportPanel: React.FC<ImportPanelProps> = ({ timeFormats, onImportExcel }) => {
      ...
      <div className="bg-white rounded-lg shadow p-6">
         ...
         <div className="bg-yellow-50 rounded-lg border border-yellow-200 p-4 h-[calc(100%-5rem)] overflow-y-auto">
             ...
             <ul className="text-sm text-yellow-700 space-y-2 list-disc list-inside">
                {timeFormats.map((fmt) => (
                    <li key={fmt.id} title={fmt.pattern}><span className="font-mono">{fmt.name}</span></li>
                ))}
                <li className="pt-2 border-t border-yellow-200 mt-2 text-yellow-600 italic">注：支持括号内的备注信息</li>
             </ul>
         </div>
      </div>
    </div>
...
const IssueSidebar: React.FC<IssueSidebarProps> = ({ 
  events, locations, onFixSerial, onAddLocationToLibrary, onAddFormatRule,
  onUpdateEventTime, onUpdateEventLocation, onIgnoreError, onRestoreError, selection, onSelectIssue
}) => {
  ...
  return (
    <div className="w-[450px] bg-white border-l border-gray-200 flex flex-col h-full sticky top-20 overflow-y-auto shadow-lg rounded-l-xl transition-all duration-300">
      ...
      <div className="p-4 space-y-6">
        {viewMode === 'pending' && (
          <>
            {pendingCount === 0 && <div className="text-center py-10 text-gray-500 text-sm"><Check className="w-8 h-8 mx-auto mb-2 text-green-500" />所有问题已处理</div>}
            {pendingSerialIssues.length > 0 && (
              <div id="issue-serial-group" onClick={() => onSelectIssue?.(pendingSerialIssues[0].id, 'sidebar', 'serial')} className={`bg-white rounded-lg border border-gray-200 shadow-sm p-4 relative group cursor-pointer ${highlightId === 'issue-serial-group' ? 'ring-2 ring-orange-400 bg-orange-50' : ''}`}>
                <div className="flex items-center mb-3 text-orange-600 justify-between">
                  <div className="flex items-center"><ListOrdered className="w-5 h-5 mr-2" /><h4 className="font-bold text-base">序号不连续</h4></div>
                  <button onClick={(e) => { e.stopPropagation(); pendingSerialIssues.forEach(e => onIgnoreError(e.id, 'serial')); }} className="text-xs text-gray-500 underline">全部忽略</button>
                </div>
                <p className="text-sm text-gray-600 mb-4">检测到 {pendingSerialIssues.length} 个序号与实际行号不匹配。</p>
                <button onClick={(e) => { e.stopPropagation(); onFixSerial(); }} className="w-full flex items-center justify-center px-4 py-2 bg-orange-50 text-orange-700 text-sm font-semibold rounded-md border border-orange-200 hover:bg-orange-100"><Wand2 className="w-4 h-4 mr-2" />一键重排序号</button>
              </div>
            )}
            {uniqueInvalidLocations.length > 0 && (
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
                <div className="flex items-center justify-between mb-3 text-red-600">
                  <div className="flex items-center"><MapPin className="w-5 h-5 mr-2" /><h4 className="font-bold text-base">未知地点 ({uniqueInvalidLocations.length})</h4></div>
                  <button onClick={() => uniqueInvalidLocations.forEach(loc => onAddLocationToLibrary(loc))} className="text-xs text-blue-600 underline">全部入库</button>
                </div>
                    ...
                    return (
                      <div key={idx} data-loc-card={loc} onClick={() => affectedEvents.length > 0 && onSelectIssue?.(affectedEvents[0].id, 'sidebar', 'location')} className={`bg-gray-50 p-3 rounded-md text-sm border border-gray-100 shadow-sm cursor-pointer ${highlightId === loc ? 'ring-2 ring-blue-400 bg-blue-50' : ''}`}>
                        <div className="flex justify-between items-start gap-3 mb-2">
                             <span className="font-bold text-gray-800 break-words flex-1 leading-snug">{loc}</span>
                             <div className="flex flex-col gap-2 shrink-0">
                                <button onClick={(e) => { e.stopPropagation(); onAddLocationToLibrary(loc); }} className="px-2 py-1.5 rounded text-xs font-medium text-blue-700 bg-blue-100 border border-blue-200 w-16"><Plus className="w-3.5 h-3.5 mr-1 inline"/>入库</button>
                                <button onClick={(e) => { e.stopPropagation(); affectedEvents.forEach(e => onIgnoreError(e.id, 'location')); }} className="px-2 py-1.5 rounded text-xs font-medium text-gray-700 bg-white border border-gray-300 w-16"><EyeOff className="w-3.5 h-3.5 mr-1 inline"/>忽略</button>
                             </div>
                        </div>
                        {recommendation && (
                           <div className="mt-2 flex items-center justify-between bg-yellow-50 p-2 rounded border border-yellow-100">
                               <div className="flex items-center text-yellow-800 text-xs"><Lightbulb className="w-3.5 h-3.5 mr-1.5" />建议: {recommendation}</div>
                               <button onClick={(e) => { e.stopPropagation(); affectedEvents.forEach(evt => onUpdateEventLocation(evt.id, recommendation)); }} className="text-yellow-700 text-xs font-bold underline">修改</button>
                           </div>
                        )}
                      </div>
                    );
            ...
            {pendingTimeIssues.length > 0 && (
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
                <div className="flex items-center mb-3 text-red-600"><Clock className="w-5 h-5 mr-2" /><h4 className="font-bold text-base">时间格式错误 ({pendingTimeIssues.length})</h4></div>
                    ...
                    return (
                      <div key={item.id} id={`issue-time-card-${item.id}`} onClick={() => onSelectIssue?.(item.id, 'sidebar', 'time')} className={`bg-gray-50 p-3 rounded-md border border-gray-100 shadow-sm cursor-pointer ${isHighlighted ? 'ring-2 ring-red-400 bg-red-50' : ''}`}>
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex flex-col flex-1 mr-2"><span className="text-sm font-bold text-gray-800 leading-snug">{item.name}</span><span className="text-xs text-gray-400 mt-0.5">序号 #{item.serialNo}</span></div>
                        </div>
                        <div className="flex items-center gap-2">
                           {editingTimeId === item.id ? (
                              <div className="flex gap-1 flex-1">
                                  <input autoFocus type="text" onClick={(e) => e.stopPropagation()} className="flex-1 text-xs border border-blue-300 rounded px-2 py-1.5 outline-none bg-white" value={tempTimeVal} onChange={(e) => setTempTimeVal(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveTime(item.id)} />
                                  <button onClick={(e) => { e.stopPropagation(); handleSaveTime(item.id); }} className="bg-green-100 text-green-700 px-2 py-1 rounded border border-green-200"><Check className="w-4 h-4" /></button>
                              </div>
                            ) : (
                              <div className="flex-1 text-sm text-red-600 bg-red-50 px-3 py-1.5 rounded cursor-pointer border border-red-100 group h-9 flex items-center justify-between" onClick={(e) => { e.stopPropagation(); handleStartEditTime(item.id, item.time); }} title="点击修改">
                                  <span className="truncate">{item.time}</span><Wand2 className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 text-red-400" />
                              </div>
                            )}
                            <div className="flex gap-2 shrink-0">
                                <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (canAddRule) handleAddFormatRule(item.time); }} className={`px-3 py-1.5 rounded text-xs font-medium flex items-center shadow-sm h-9 ${canAddRule ? "text-blue-700 bg-blue-100 border border-blue-200" : "text-gray-400 bg-gray-100 cursor-not-allowed border border-gray-200"}`} title={canAddRule ? "将此格式加入规则库" : "无法入库"}><FilePlus className="w-3.5 h-3.5 mr-1" />入库</button>
                                <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); onIgnoreError(item.id, 'time'); }} className="px-3 py-1.5 rounded text-xs font-medium text-gray-700 bg-white border border-gray-300 h-9 flex items-center"><EyeOff className="w-3.5 h-3.5 mr-1"/>忽略</button>
                            </div>
                        </div>
                        {item.validationMessage && <div className="mt-1.5 text-xs text-red-500 flex items-start"><AlertCircle className="w-3 h-3 mr-1 mt-0.5" /><span>{item.validationMessage}</span></div>}
                        {recommendation && (
                           <div className="mt-2 flex items-center justify-between bg-yellow-50 p-2 rounded border border-yellow-100">
                               <div className="flex items-center text-yellow-800 text-xs"><Lightbulb className="w-3.5 h-3.5 mr-1.5" />建议: {recommendation}</div>
                               <button onClick={(e) => { e.stopPropagation(); onUpdateEventTime(item.id, recommendation); }} className="text-yellow-700 text-xs font-bold underline">修改</button>
                           </div>
                        )}
                      </div>
                    );
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    setEvents(prevEvents => prevEvents.map(evt => {
       ...
       return { 
           ...evt, isLocationValid, isTimeValid: timeValidation.isValid, validationMessage: timeValidation.message 
       };
    }));
  }, [locations, timeFormats]);

  const addTimeFormat = (name: string, pattern: string) => {
    setTimeFormats(prev => [...prev, { id: Math.random().toString(36).substr(2, 9), name, pattern }]);
  };
  const removeTimeFormat = (id: string) => { setTimeFormats(prev => prev.filter(t => t.id !== id)); };
  ...
  const updateEventTime = (id: string, newTime: string) => handleUpdateEvent(id, 'time', newTime);
  const updateEventLocation = (id: string, newLocation: string) => handleUpdateEvent(id, 'location', newLocation);
  ...
  const handleExcelImport = (rawData: any[]) => {
    if (events.length > 0) {
        if (!window.confirm("当前页面存在已有数据，继续导入将删除已有数据按最新的文件内容重新导入")) return;
    }
      ...
      <header className="bg-white shadow-sm z-20 sticky top-0">
        <div className="max-w-[95%] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center"><LayoutDashboard className="w-8 h-8 text-indigo-600" /><span className="ml-2 text-xl font-bold text-gray-900">活动数据校验助手</span></div>
            <nav className="flex space-x-4 items-center">
              ...
              <button onClick={() => setView('time-formats')} className={`px-3 py-2 rounded-md text-sm font-medium transition-colors flex items-center ${view === 'time-formats' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-gray-700'}`}><Clock className="w-4 h-4 mr-1.5" />时间格式</button>
            </nav>
          </div>
        </div>
      </header>
          ...
          <div className="flex gap-6 items-start">
            <div className={`flex-1 min-w-0 transition-all duration-300 ${hasIssues ? 'max-w-[calc(100%-30rem)]' : 'w-full'}`}>
              <div className="space-y-8">
                <div className="flex justify-between items-center">
                    <button onClick={() => setIsImportPanelOpen(!isImportPanelOpen)} className="flex items-center text-sm font-medium text-gray-700 bg-white px-3 py-2 rounded-md border border-gray-300 shadow-sm hover:bg-gray-50">
                      {isImportPanelOpen ? <>收起导入面板 <ChevronUp className="w-4 h-4 ml-2" /></> : <>展开导入面板 <ChevronDown className="w-4 h-4 ml-2" /></>}
                    </button>
                    <button onClick={exportExcel} className="flex items-center px-4 py-2 bg-green-600 text-white rounded-md shadow-sm text-sm font-medium hover:bg-green-700 focus:outline-none"><Download className="w-4 h-4 mr-2" />导出 Excel</button>
                </div>
            ...
            {hasIssues && (
              <div className="flex-shrink-0 sticky top-24">
                <IssueSidebar events={events} locations={locations} onFixSerial={fixSerialNumbers} onAddLocationToLibrary={addLocation} onAddFormatRule={addTimeFormat} onUpdateEventTime={updateEventTime} onUpdateEventLocation={updateEventLocation} onIgnoreError={handleIgnoreError} onRestoreError={handleRestoreError} selection={selection} onSelectIssue={(id, source, field) => setSelection({ id, source, field })} />
              </div>
            )}
          </div>
        ) : view === 'library' ? (
